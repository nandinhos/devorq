...
> Atualizado em 2026-08-31 (audit + fixes C1–C5/M8/T1 + **Blocos 3 e 4 concluídos nesta sessão**). Repo: `github.com/nandinhos/devorq` · branch `main`.
...
**✅ Blocos 3 e 4 CONCLUÍDOS nesta sessão** (Bloco 3: M4/M5/M9/M10; Bloco 4: M1/M2/M3 + alinhamento de escopos). Trilha completa em §2.

**Bloco 5 — robustez (PRÓXIMO):**
1. **M6 (jq opcional):** `lib/commands/brainstorm.sh`(11), `grill.sh`(15), `lib/commit.sh`(4), `lib/rules.sh` usam `jq` sem guard `devorq::contracts::require_jq` — a filosofia é "jq opcional". Aplicar o guard.
2. **M7 (portabilidade GNU/BSD/macOS):** `timeout` (`delegate.sh:139`, `gates.sh:538`), `realpath -q` (`helpers.sh:45,50`, `vps.sh:61,65`), `readlink -f` (`e2e-test.sh:11`) ausentes no macOS — prever fallback BSD/GNU.
3. **M12/M13 (loop AUTO):** `lib/loop.sh:157-159` valida só `-z "$executor"` (não `-x`); `exit_code` do contrato v1 hardcoded; `mark_skip` (`loop-auto.sh`) faz `mv` sem validar JSON; `MAX_DELEGATE_RETRIES=1` (retry morto); `progress.txt` não excluído do commit.

**Baixos/higiene** — ver §2 e §7 (references órfãs, mojibake, `INSTALL.md` typos, `unit::skip` indefinido, `systematic_debug` stub, versão `devorq-auto` inconsistente).

Commit (convenção): `fix(core): ...` / `fix(loop): ...`.
